...
pub fn handle_death_effects(
    ...
    for event in death_events.read() {
        if let Ok((transform, team)) = query.get(event.entity) {
            ...
            commands.entity(event.entity).insert(DeathEffect {
                fade_time: 1.0,
                time_elapsed: 0.0,
            ...
            bevy::log::info!("Unit died at {:?}", transform.translation);
        }
    }
}
...
pub fn update_death_effects(
    mut commands: Commands,
    time: Res<Time>,
    mut query: Query<(Entity, &mut DeathEffect, &MeshMaterial3d<StandardMaterial>)>,
    mut materials: ResMut<Assets<StandardMaterial>>,
) {
...
pub fn update_combat_particles(
    mut commands: Commands,
    time: Res<Time>,
    mut query: Query<(Entity, &mut Transform, &mut CombatParticle)>,
) {
...
pub fn cleanup_expired_effects(
    mut commands: Commands,
    particles: Query<Entity, With<CombatParticle>>,
    damage_nums: Query<Entity, With<DamageNumber>>,
) {
